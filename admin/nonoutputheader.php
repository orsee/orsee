...
if ($settings__stop_admin_site=="y" && $document!="error_temporaly_disabled.php") redirect("errors/error_temporaly_disabled.php");

// Check for login
if ((!$expadmindata['adminname']) && ($document!="admin_login.php")) redirect ("admin/admin_login.php");

if (isset($_REQUEST['new_language'])) {
        $expadmindata['language']=$_REQUEST['language'];
        $_SESSION['expadmindata']=$expadmindata;
}

if (!isset($expadmindata['language'])) $expadmindata['language']=$settings['admin_standard_language'];
