...
// Include the following if GD < 2.0:

if (!(isset($settings['stats_plots_gd_version']) && $settings['stats_plots_gd_version']==2)) {

define('IMG_ARC_PIE',0);
define('IMG_ARC_CHORD',1);
define('IMG_ARC_NOFILL',2);
define('IMG_ARC_EDGED',4);
function ImageFilledArc2 ($im,$cx,$cy,$diameter,$diam2,$s,$e,$col,$opts) {

        $i=1; $j=1; $jexact=1;
        while ($i<$diameter || $j<$diam2) {
        	ImageArc($im, $cx, $cy,$i,$j,$s,$e,$col);
        	$i++;
        	if ($j < $diam2) {
			$jexact=$jexact + $diam2/$diameter;
			$j= (int) round($jexact);
			}
        	}
	}
...
function ImageFilledArc2 ($im,$cx,$cy,$diameter,$diam2,$s,$e,$col,$opts) {
	ImageFilledArc($im,$cx,$cy,$diameter,$diam2,$s,$e,$col,$opts);
	}
}
...
//Colors and styles       (all colors can be array (R,G,B) or named color)
    ...
    var $data_colors = array('SkyBlue', 'green', 'orange', 'blue', 'red', 'violet', 'azure1');
    var $error_bar_colors = array('SkyBlue', 'green', 'orange', 'blue', 'red', 'violet', 'azure1');
...
/////////////////////////////////////////////
    ...
    function DrawPieChart() 
    {
        ...
        for ($h = $this->shading; $h >= 0; $h--) {
            ...
            foreach ($sumarr as $val) {
                ...
                // Draw the slice       
                ImageFilledArc2($this->img, $xpos, $ypos+$h, $diameter, $diam2, 
    ...
    function DrawDot($x_world, $y_world, $dot_type, $color) 
    {
        ...
        case 'circle':
            ...
            break;
        case 'dot':
            ImageFilledArc2($this->img, $x_mid, $y_mid, $this->point_size, $this->point_size, 0, 360, 
                           $color, IMG_ARC_PIE);
            break;
        case 'diamond':
